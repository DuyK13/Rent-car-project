...
public class LoginRequestValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        } else if (!userService.isCorrectPassword(loginRequest.getUsername(), loginRequest.getPassword())) {
            errors.rejectValue("password", "password.wrong", "User " + loginRequest.getUsername() + " wrong password");
        } else if (!userService.isUserEnable(loginRequest.getUsername())) {
            errors.rejectValue("username", "username.inactive", "User " + loginRequest.getUsername() + " is not enable or block");
        }
    }
